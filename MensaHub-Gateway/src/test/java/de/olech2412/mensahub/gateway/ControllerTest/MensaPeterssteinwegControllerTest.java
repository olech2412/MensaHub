...
public class MensaPeterssteinwegControllerTest {
    ...
    String mensaName = "mensa_peterssteinweg";
    ...
    @Autowired
    Meals_Mensa_PeterssteinwegService mealsMensaPeterssteinwegService;

    @Autowired
    Mensa_PeterssteinwegService mensaPeterssteinwegService;
    ...
    public void contextLoads() {
        ...
        assertThat(mealsMensaPeterssteinwegService).isNotNull();
        assertThat(mensaPeterssteinwegService).isNotNull();
    }
    ...
    public void controllerShouldProvideMensaName() throws Exception {
        ...
        Assertions.assertEquals("{\"name\":\"" + mensaPeterssteinwegService.getMensa().getName() + "\"}", content);
    }
    ...
    public void controllerShouldProvideMealsForSpecificCategory() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/category/Pastateller").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaPeterssteinwegService.findAllByCategory("Pastateller").size())));
    }
    ...
    public void controllerShouldProvideMealsForSpecificCategoryAndServingDate() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/category/Testkategorie3/servingDate/2001-03-03").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaPeterssteinwegService.findAllByCategoryAndServingDate("Testkategorie3", LocalDate.parse("2001-03-03")).size())));
    }
    ...
    public void controllerShouldProvideMealsWhereRatingIsLessThen() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/byRatingLessThen/" + randomRating).header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaPeterssteinwegService.findAllByRatingLessThanEqual(Double.valueOf(randomRating)).size())));
    }
    ...
    public void controllerShouldProvideMealsWhereRatingIsHigherThen() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/byRatingHigherThen/" + randomRating).header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaPeterssteinwegService.findAllByRatingGreaterThanEqual(Double.valueOf(randomRating)).size())));
    }

    @Test
    public void controllerShouldReceivePostData() throws Exception {
        Meals_Mensa_Peterssteinweg testMealBeforePost = (Meals_Mensa_Peterssteinweg) mealsMensaPeterssteinwegService.findByNameAndServingDateAndId("Testname", LocalDate.parse("2001-01-01"), 1L).get(0);
        this.mockMvc.perform(post("/" + mensaName + "/sendRating").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken()).contentType(MediaType.APPLICATION_JSON).content(jsonData))
                ...
                .andExpect(status()
                        .isOk()).andReturn();
        Meals_Mensa_Peterssteinweg testMealAfterPost = (Meals_Mensa_Peterssteinweg) mealsMensaPeterssteinwegService.findByNameAndServingDateAndId("Testname", LocalDate.parse("2001-01-01"), 1L).get(0);
