...
import de.mensahub.gateway.JPA.services.meals.Meals_Mensa_am_MedizincampusService;
import de.mensahub.gateway.JPA.services.mensen.Mensa_am_MedizincampusService;
...
public class MensaamMedizincampusControllerTest {
    ...
    String mensaName = "mensa_am_medizincampus";
    ...
    @Autowired
    Meals_Mensa_am_MedizincampusService mealsMensaAmMedizincampusService;

    @Autowired
    Mensa_am_MedizincampusService mensaAmMedizincampusService;
    ...
    public void contextLoads() {
        ...
        assertThat(mealsMensaAmMedizincampusService).isNotNull();
        assertThat(mensaAmMedizincampusService).isNotNull();
    }
    ...
    public void controllerShouldProvideMensaName() throws Exception {
        ...
        Assertions.assertEquals("{\"name\":\"" + mensaAmMedizincampusService.getMensa().getName() + "\"}", content);
    }
    ...
    public void controllerShouldProvideMealsForSpecificCategory() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/category/Pastateller").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaAmMedizincampusService.findAllByCategory("Pastateller").size())));
    }
    ...
    public void controllerShouldProvideMealsForSpecificCategoryAndServingDate() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/category/Testkategorie3/servingDate/2001-03-03").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaAmMedizincampusService.findAllByCategoryAndServingDate("Testkategorie3", LocalDate.parse("2001-03-03")).size())));
    }
    ...
    public void controllerShouldProvideMealsWhereRatingIsLessThen() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/byRatingLessThen/" + randomRating).header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaAmMedizincampusService.findAllByRatingLessThanEqual(Double.valueOf(randomRating)).size())));
    }
    ...
    public void controllerShouldProvideMealsWhereRatingIsHigherThen() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/" + mensaName + "/byRatingHigherThen/" + randomRating).header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasItems()))
                .andExpect(jsonPath("$", hasSize(mealsMensaAmMedizincampusService.findAllByRatingGreaterThanEqual(Double.valueOf(randomRating)).size())));
    }

    @Test
    public void controllerShouldReceivePostData() throws Exception {
        Meals_Mensa_am_Medizincampus testMealBeforePost = (Meals_Mensa_am_Medizincampus) mealsMensaAmMedizincampusService.findByNameAndServingDateAndId("Testname", LocalDate.parse("2001-01-01"), 1L).get(0);
        this.mockMvc.perform(post("/" + mensaName + "/sendRating").header(HttpHeaders.AUTHORIZATION,"Bearer " + getJWTToken()).contentType(MediaType.APPLICATION_JSON).content(jsonData))
                ...
                .andExpect(status()
                        .isOk()).andReturn();
        Meals_Mensa_am_Medizincampus testMealAfterPost = (Meals_Mensa_am_Medizincampus) mealsMensaAmMedizincampusService.findByNameAndServingDateAndId("Testname", LocalDate.parse("2001-01-01"), 1L).get(0);
