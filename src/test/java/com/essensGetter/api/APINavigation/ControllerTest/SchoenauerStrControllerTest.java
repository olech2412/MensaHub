...
public class SchoenauerStrControllerTest {
    ...
    public void controllerShouldReturnMealData() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/getMeals/mensa_schoenauer_str?code=8PLUv50emD7jBakyy9U4").contentType(MediaType.APPLICATION_JSON)).andDo(print())
    ...
    public void controllerShouldBeAccessedOnlyWithAuthCode() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/getMeals/mensa_schoenauer_str")).andDo(print()).andExpect(status().is(401));
    }

    @Test
    public void controllerShouldBeAccessedOnlyWithValidAuthCode() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/getMeals/mensa_schoenauer_str?code=" + RandomString.make(20))).andDo(print()).andExpect(status().is(401));
    }
    ...
    public void controllerShouldReceivePostData() throws Exception {
        ...
        this.mockMvc.perform(post("/sendRating/mensa_schoenauer_str?code=8PLUv50emD7jBakyy9U4").contentType(MediaType.APPLICATION_JSON).content(jsonData))
                .andDo(print())
                .andExpect(status()
                .isOk()).andReturn();
