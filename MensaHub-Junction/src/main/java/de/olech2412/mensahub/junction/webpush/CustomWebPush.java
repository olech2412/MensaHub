...
public class CustomWebPush {
     ...
     */
    public CustomWebPush(String publicKey, String privateKey, String subject) {
        try {
            ...
            log.error("Error while checking if WebPush is enabled because there is no VaadinService existing");
        }
    ...
    public void sendNotification(Subscription subscription,
            CustomWebPushMessage message) throws WebPushException {
        int statusCode = -1;
    ...
    private void initWebPushClient(UI ui) {
        ...
        try (InputStream stream = CustomWebPush.class.getClassLoader()
    ...
    private Logger getLogger() {
        return LoggerFactory.getLogger(CustomWebPush.class);
    }
}
