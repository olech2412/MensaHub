...
self.addEventListener('push', (e) => {
    ...
    if (data) {
        self.registration.showNotification(data.title, {
            body: data.body,
            icon: data.icon || '/icons/icon.png', // Verwende das übergebene Icon oder ein Standard-Icon
            data: {
                url: data.url // Speichere die URL in den Benachrichtigungsdaten
            }
        });
    }
});
...
async function focusOrOpenWindow(url: string) {  // Typ für 'url' festgelegt
    const allWindows = await self.clients.matchAll({
        type: 'window',
        includeUncontrolled: true
    });

    // Suche nach einem Fenster, das die URL bereits geöffnet hat
    const matchingWindow = allWindows.find((w) => w.url === url);

    if (matchingWindow) {
        return matchingWindow.focus(); // Fokussiere das vorhandene Fenster
    } else {
        return self.clients.openWindow(url); // Öffne ein neues Fenster mit der URL
    }
}